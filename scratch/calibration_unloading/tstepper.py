...
from traits.api import HasTraits, Instance, \
    Property, cached_property, Float, List, Any
...
#     material = Any
    ...
    def apply_bc(self, step_flag, K_mtx, F_ext, t_n, t_n1):
        '''Apply boundary conditions for the current load increment
        '''
        for bc in self.bc_list:
    ...
    def get_corr_pred(self, step_flag, U, d_U, eps, sig, t_n, t_n1, alpha, q, kappa):
        ...
        # d_u_e--the displacement incremental vector of each element
        d_u_e = d_U[elem_dof_map]
        #[n_e, n_dof_r, n_dim_dof]
        d_u_n = d_u_e.reshape(n_e, n_dof_r, n_nodal_dofs)
        #[n_e, n_ip, n_s] d_eps--the incremental strain at each point
        ...
        F_ext_record = -F_int.copy()
        self.apply_bc(step_flag, self.K, F_int, t_n, t_n1)
        return F_int, self.K, eps, sig, alpha, q, kappa, F_ext_record
