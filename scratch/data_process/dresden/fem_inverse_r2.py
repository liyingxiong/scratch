...
class FETS1D52ULRH(FETSEval):
    ...
    A_m = Float(60 * 9 - 1.85, desc='matrix area [mm2]')
...
if __name__ == '__main__':
    ...
    length = {'6-1': 18.4, '6-2': 21.6, '7-1': 20.1, '7-2': 19.9, '8-1': 19.6, '8-2': 19.2,
              '9-1': 20, '9-2': 19.2, '10-1': 20.4, '10-2': 20.2}

    delta_f = {'6-1': 13.9, '6-2': 14.0, '7-1': 13.9, '7-2': 13.9, '8-1': 13.7, '8-2': 13.7,
               '9-1': 13.6, '9-2': 13.6, '10-1': 13.4, '10-2': 13.5}
    ...
    for fname in listdir(fpath):
        ...
        x = (data[1] + data[2]) / 2.
        y = data[3] - delta_f[l] * data[3] / np.amax(data[3])
        ...
        x = np.hstack((0, np.linspace(0.01, 2.5, 1000)))
...
#             (np.linspace(0, 0.109, 2), np.linspace(0.13, 0.30, 10), np.linspace(0.35, 1.5, 8)))
        ...
        w_arr = np.hstack(
            (np.linspace(0, 0.05, 6), np.linspace(0.08, 2.5, 20)))
