...
class RandomField(HasTraits):
    ...
    lacor = Float(1.0 , auto_set = False, enter_set = True,
    ...
    length = Float(desc = 'length of field', field = True)
    n_p = Int(desc = 'number of points in the field', field = True)
    distribution = Enum('Gauss', 'Weibull', modified = True)
    
    xgrid = Property(Array, depends_on='+field')
    @cached_property
    def _get_xgrid(self):
        return np.linspace(0, self.length, self.n_p)
